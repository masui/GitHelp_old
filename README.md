...
* オンラインヘルプは以下のような問題がある。
  1. **ヘルプエントリがみつからない**
  2. ヘルプでやり方がわかったとしても**パラメタを指定して実行しなおす必要がある**
* どのようなキーワードで検索してもみつかるように正規表現でいろんな表現を用意しておき、やり方がわかればすぐ実行できるというのが
本方式の良いところである。
* 「README.mdを消す方法」はマニュアルには書いてない。ファイルを消す方法を探す必要がある。
...
githelpはコマンドラインから使えるので毎日使えるかもしれない。
